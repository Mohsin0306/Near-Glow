...
exports.createCategory = async (req, res) => {
  try {
    ...
    const { name, description, icon, color, bgPattern } = req.body;
    
    // Parse JSON strings back to arrays
    const subcategories = req.body.subcategories ? JSON.parse(req.body.subcategories) : [];
    const featured = req.body.featured ? JSON.parse(req.body.featured) : [];
    ...
    const category = new Category({
      name,
      description,
      icon: icon || 'RiFlowerLine',
      color: color || 'from-pink-400 to-rose-500',
      bgPattern: bgPattern || 'bg-[radial-gradient(circle_at_top_right,_var(--tw-gradient-from),transparent_70%)]',
      subcategories,
      featured,
      image: {
...
exports.updateCategory = async (req, res) => {
  try {
    ...
    const iconName = req.body.icon ? 
      (typeof req.body.icon === 'object' ? req.body.icon.name : req.body.icon) 
      : category.icon;

    // Parse arrays if they're strings, otherwise use the arrays directly
    const subcategories = req.body.subcategories ? 
      (Array.isArray(req.body.subcategories) ? req.body.subcategories : JSON.parse(req.body.subcategories))
      : category.subcategories;
    
    const featured = req.body.featured ?
      (Array.isArray(req.body.featured) ? req.body.featured : JSON.parse(req.body.featured))
      : category.featured;
